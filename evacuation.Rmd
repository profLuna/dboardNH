...
NFHZAbyTown <- nh_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(nh_towns, st_crs(nh_blkgrps_nfhza))) %>% 
...
# Calculate Pop and Pct pop at risk by jurisdiction
HEVACbyTown <- nh_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(nh_towns, st_crs(nh_blkgrps_hevac))) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
town_floodPop_pct <- town_nfhzaPop_pct %>% 
  left_join(., HEVACbyTown, by = "NAMELSAD") %>% 
  replace_na(list(hevacPop = 0)) %>% 
  mutate(PctPopHEVAC = if_else(totalpopE == 0, 0, hevacPop/totalpopE*100))
...
house_names_pops <- get_acs(geography = "block group", survey = "acs5",
                     variables = c(totalpop = "B03002_001"), state = "NH", 
                     output = "wide", year = 2018, geometry = TRUE) %>% 
  select(GEOID, totalpopE) %>% 
  st_transform(.,st_crs(nh_blkgrps_nfhza)) %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(house_districts,st_crs(nh_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_floodPop_pct <- nh_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(house_districts, st_crs(nh_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
senate_names_pops <- get_acs(geography = "block group", survey = "acs5",
  ...
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(senate_districts,st_crs(nh_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
# identify FEMA risk pops by district and calculate pop pct
NFHZAbySenate <- nh_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(nh_blkgrps_nfhza))
...
# identify HEVAC risk pops by district and calculate pop pct, and bring together with FEMA
senate_floodPop_pct <- nh_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(nh_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
muniNFHZA <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsNFHZA = n())

houseNFHZA <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsNFHZA = n())

senateNFHZA <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsNFHZA = n())
...
muniHEVAC <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsHEVAC = n())

houseHEVAC <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsHEVAC = n())

senateHEVAC <- nh_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsHEVAC = n())
...
nh_towns <- nh_towns %>% 
  left_join(., muniNFHZA, by = c("NAMELSAD" = "TOWN")) %>%
  left_join(., muniHEVAC, by = c("NAMELSAD" = "TOWN")) %>%
...
# create layer of nh_blkgrps_sf for flood and hurricane
nh_blkgrps_sf_NFHZA <- nh_blkgrps_sf %>% 
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, floodtypes) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

nh_blkgrps_sf_HEVAC <- nh_blkgrps_sf %>% 
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>% 
...
---
# Evacuation
Flood and storm surge are significant risks in New Hampshire. In the event of extensive inland or coastal flooding, evacuation may be required. For individuals and households with limited mobility, either due to inadequate access to transportation options or because of physical limitations, evacuation presents heightened risk. Evacuation may also prove especially difficult for individuals and households due to limited economic resources, difficulty understanding or accessing information, or low trust in official sources of information. 

* Over 78,000 people in New Hampshire live in a FEMA flood risk zone and over 14,000 live in a hurricane evacuation zone. 
* Households without a car and low income persons are more likely to live in flood zones compared to the general population or other priority populations. 
* Those over 64 years of age and households without a car are more likely to live in a hurricane evacuation zone compared to the general population or other priority populations.

These interactive figures identify communities across New Hampshire that are most most vulnerable to the risks of inland flooding or coastal storm surge.
...
PopupHouse <- paste0("New Hampshire state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

PopupSenate <- paste0("New Hampshire state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

Popup <- paste0(nh_blkgrps_sf_NFHZA$NAME, "<br/>",
                "<b>State House District:</b> ", nh_blkgrps_sf_NFHZA$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", nh_blkgrps_sf_NFHZA$SEN_DIST, "<br/>",
                "<b>Town:</b> ", nh_blkgrps_sf_NFHZA$TOWN, "<br/>",
                "<b>Priority Populations: </b>", nh_blkgrps_sf_NFHZA$POPSlabel, "<br/>",
                "<b>FEMA flood risks: </b> ", nh_blkgrps_sf_NFHZA$floodtypes)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = nh_blkgrps_sf_NFHZA, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
PopupHouse <- paste0("New Hampshire state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

PopupSenate <- paste0("New Hampshire state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

Popup <- paste0(nh_blkgrps_sf_HEVAC$NAME, "<br/>",
                "<b>State House District:</b> ", nh_blkgrps_sf_HEVAC$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", nh_blkgrps_sf_HEVAC$SEN_DIST, "<br/>",
                "<b>Town:</b> ", nh_blkgrps_sf_HEVAC$TOWN, "<br/>",
                "<b>Priority Populations: </b>", nh_blkgrps_sf_HEVAC$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = senate_districts,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "State Senate Districts") %>% 
  addPolygons(data = nh_blkgrps_sf_HEVAC, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
*Note that digital data on flood risk around Laconia is not currently available from FEMA. Flood risk for areas around Laconia is likely to be underestimated.*

The analysis of hurricane evacuation risk presented here is based on Hurricane Evacuation Maps produced by the [U.S. Army Corps of Engineers for New England](https://www.nae.usace.army.mil/Missions/Projects-Topics/New Hampshire-Hurricane-Studies/). The maps show Hurricane Evacuation Zones that are recommended to be evacuated during potential worst-case hurricane storm surge inundation. These evacuation zones are based on storm surge modeling by the National Weather Service’s National Hurricane Center using the hydrodynamic [Sea, Lake, and Overland Surges from Hurricanes (SLOSH) model](https://www.nhc.noaa.gov/nationalsurge/) to simulate storm surge from tropical cyclones. 
...
nh_FloodPops_df %>% 
  mutate(
    Status = case_when(
           ...
           PctFlood < nh_FloodPops_df[nh_FloodPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctFlood > nh_FloodPops_df[nh_FloodPops_df$Group=="Total Pop",4] ~ "Above state avg"),
  ...
  hc_title(text = "New Hampshire Populations Living within Flood Zones", useHTML = TRUE)
```
...
```{r graphHEVAC, fig.align="center", fig.cap="New Hampshire Populations Living within Hurricane Evacuation Zones."}
# Add status variable and recode demographic names
nh_HevacPops_df %>% 
  mutate(
    Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctHevac < nh_HevacPops_df[nh_HevacPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctHevac > nh_HevacPops_df[nh_HevacPops_df$Group=="Total Pop",4] ~ "Above state avg"),
    Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma"),
  ...
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}%") %>% 
  hc_title(text = "New Hampshire Populations Living within Hurricane Evacuation Zones", useHTML = TRUE)
```
...
These tables show the number and percentage of the general population by jurisdiction exposed to FEMA flood risk or hurricane evacuation risk. FEMA flood risk is calculated as the percentage of a Census block group that falls within 100-year or 500-year flood risk zones and aggregated by jurisdiction. Hurricane evacuation risk is calculated as the percentage of a Census block group that falls within hurricane evacuation zones identified by the New Hampshire Emergency Management Agency and U.S. Army Corps of Engineers, and aggregated by jurisdiction. Population values are derived from the American Community Survey 5-year estimates for 2014-2018.
